...
$item['option'] = $variation['options'];
  ...
  function add_item($store_id, $item_id, $item_qty=1, $item_price, $item_info=array('name'=>"",'options'=>array())) {
    ...
    if ($valid_item_qty && $valid_item_price) {
      ...
      //break apart store_id and store_name
      if ($store_id !== '') {
        $pos = strrpos($store_id, ',');
        if ($pos !== false) {
          $this->storenames[substr($store_id,0,$pos)] = substr($store_id,$pos+1);
          $store_id = substr($store_id,0,$pos);
        } else {
          if (!$this->storenames[$store_id]) {
            $this->storenames[$store_id] = '';
          }
        }
        unset($pos);
      }
/*
$store_name = explode(',', $store_id);
if (sizeof($store_name) > 1) {
  $this->storenames[$store_name[0]] = $store_name[1];
  $store_id = $store_name[0];
}
unset($store_name);
*/
      ...
      foreach($this->items as $store) {
        ...
        foreach($item['variation'] as $variation) {
          ...
          foreach($item['price'] as $key => $val) {
            if($variation['qty'] >= $key){ $discount_base = $key; } else { break; }
          }
          $variation_price += ($item['price'][$discount_base] + $option_price) * $variation['qty'];
          unset($discount_base, $option_price);
...
//bulk
  ...
  function _getstorename($which) {
    return $this->storenames[$which];
  }
